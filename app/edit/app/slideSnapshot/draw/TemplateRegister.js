define(["handlebars", "common/Math2", "app/deck/Utils"],
function(Handlebars, Math2, DeckUtils) {
					...
					if (ignore.indexOf(componentModel.get("type")) != -1){
						switch (componentModel.get("type")) {
							...
							case "WebFrame":
								...
								break;
							case "Audio":
                                result = JST["strut.presentation_generator.impress/AudioXM"](componentModel);
                                break;
						}
...
//						return result;
				...
				switch (componentModel.get("type")) {
					case "Image":
						if (componentModel.get("imageType") === "SVG") {
							result = JST["slideSnapshot/draw/SVGImage"](componentModel);
						} else {
							result = JST["slideSnapshot/draw/Image"](componentModel);
						}
						break;
					case "TextBox":
						result = JST["slideSnapshot/draw/TextBox"](_this.convertTextBoxData(componentModel));
						break;
					case "Table":
						result = JST["strut.presentation_generator.impress/Table"](_this.convertTextBoxData(componentModel));
						break;
					case "Video":
						if (componentModel.get("videoType") === "html5") {
							result = JST["strut.presentation_generator.impress/Video"](componentModel);
						} else {
							result = JST["strut.presentation_generator.impress/Youku"](componentModel);
						}
						break;
					case "Audio":
                        result = JST["slideSnapshot/draw/Audio"](componentModel);
                        break;
					case "WebFrame":
						result = JST["slideSnapshot/draw/WebFrame"](componentModel);
						break;
					case "Shape":
			...
			Handlebars.registerHelper("determineBG", function(slide, deck) {
				...
				if (bg && bg.indexOf('http') == 0 || bg.indexOf('http') == 0)
					return '';
				return bg;
			});
			...
			Handlebars.registerHelper("determineSurface", function(slide, deck) {
				...
				if (bg && bg != 'bg-default' && bg.indexOf('http') == -1)
					return ' ' + bg + ' ';
				return '';
			});
			...
			Handlebars.registerPartial("ComponentContainer", JST["slideSnapshot/draw/ComponentContainer"]);
